...
int main(void) {
  ...
  while (crono.campo < FINAL) {
    ...
    if (ClienteChegou()) {
      T_Item novoCliente;
      novoCliente.campo = crono.campo;
      int entrouNaFila = inserir(&fila, novoCliente);
    ...
    for (int i = 0; i < 3; i++) {
      if (!checkListaVazia(&fila) && caixa[i] == 0) {
      	client = caixa[i];
        caixa[i] = excluirInicio(&fila);

        // So exibe quando o cliente sai da fila e vai para o CAIXA.
